...
def test_l1_direct_generation_is_adapter_only(tmp_path):
    ...
    assert result.metadata["generator"] == "direct-adapter-not-configured"
    assert any(step["status"] == "not_configured" for step in result.metadata["trace_steps"])
