...
\input{figures/streaming_sequence}
...
Both models produced the same aggregate confusion matrix. All
\FinalClassifierErrors{} errors occurred in the complex-to-moderate cell. The
source-level distribution and its implications for RQ1 are examined in
\Cref{sec:rq1-classifier-discussion}.
...
\begin{table}[H]
  \centering
  \caption{Equal-dataset macro standard metrics (R-1 denotes ROUGE-1, R-2 denotes ROUGE-2, and Fact. denotes factuality; all values in percent).}
  \label{tab:primary-standard-metrics}
  \small
  \setlength{\tabcolsep}{2.9pt}
  \begin{tabularx}{\textwidth}{X*{6}{>{\centering\arraybackslash}p{0.09\textwidth}}}
    \toprule
    Configuration & \shortstack{Token\\F1} & BLEU & R-1 & R-2 & \shortstack{Context\\recall} & Fact.\\
    \midrule
