...
\label{sec:rq1-classifier-discussion}
...
precision/recall/F1. Both models made the same six record-level errors, each a
complex question predicted as moderate. This produced \FinalUnderRouting{}
under-routing and no over-routing. DistilBERT was substantially faster at
\FinalDistilBERTLatency{} versus \FinalTFiveLatency{}, while T5-small was
better calibrated (\FinalTFiveECE{} versus \FinalDistilBERTECE{} expected
calibration error). The models are therefore equal on held-out predictive
quality but differ in the latency--calibration trade-off.

The aggregate complex recall of \FinalComplexRecall{} conceals a marked source
difference. \Cref{tab:complex-errors-by-source} distributes the complex-class
validation records by their originating subset. Both classifiers correctly
identified all \FinalTemplateComplexRecords{} generated template examples but
under-routed all \FinalOfficialComplexRecords{} complex questions from the
official multi-document subsets. The official Synthetic subset contributes no
complex rows to this validation class because its selected single-document
records were labelled moderate by the preparation policy.

\begin{table}[H]
  \centering
  \caption{Complex-class validation outcomes by source subset for both final classifiers.}
  \label{tab:complex-errors-by-source}
  \begin{tabular}{lrrr}
    \toprule
    Source subset & Complex records & Correct & Predicted moderate\\
    \midrule
    Template supplement & \FinalTemplateComplexRecords & \FinalTemplateComplexCorrect & 0\\
    ExpertWritten & \FinalExpertComplexRecords & \FinalExpertComplexCorrect & 3\\
    Simulated & \FinalSimulatedComplexRecords & \FinalSimulatedComplexCorrect & 3\\
    Official Synthetic & 0 & -- & --\\
    \bottomrule
  \end{tabular}
\end{table}

Consequently, complex recall is \FinalTemplateComplexRecall{} for the template
supplement but \FinalOfficialComplexRecall{} for the six official
multi-document complex records. The missed questions are concise, naturally
phrased requests concerning slideshow controls, shopping-cart information,
business-email connection or delivery, domain ownership, and birthday-email
automation. Their complexity comes from evidence requirements rather than
from explicit words such as ``dependencies,'' ``branches,'' or ``approval
order'' that occur frequently in the generated templates. This pattern
suggests source- and template-conditioned learning that is not visible in the
overall accuracy. The official complex sample contains only six records, so it
is too small for a stable population estimate; nevertheless, the consistent
under-routing across both models is a material validity warning. Future
classifier evaluation should increase naturally phrased multi-document
complex examples, report recall by source as well as by class, and prioritize
outcome-derived route labels over template structure alone.
